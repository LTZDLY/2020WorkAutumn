...
bool loop() {
    ...
    sort(v.begin(), v.end());  //对变量按字典序排序
    int flag = 0;
    for (size_t i = 0; i < s.size(); i++) {
        if ((s[i] == '!') &&  //将单目运算符非化为双目运算符减法
    ...
    auto sss = toSuffix(ss);  //将中缀表达式转换为后缀表达式
    ...
    for (int i = 0; i < len; i++) {
        ...
        int temp = calc(sss, v, num);  //对变量赋值，后缀表达式求值
    ...
    if (mconjunctive.size() == 0)
        cout << "There is no principal conjunctive normal form in this "
                "proposition formula."
             << endl;
    else {
        cout << "The main conjunctive normal form of the proposition formula "
                "is as follows:"
             << endl;
    ...
    if (mdisjunctive.size() == 0)
        cout << "There is no principal disjunctive normal form in the "
                "propositional formula."
             << endl;
    else {
        cout << "The main disjunctive normal form of the proposition formula "
                "is as follows:"
             << endl;
