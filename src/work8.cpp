...
void Graph<T, E>::PrintGraph(ostream& os) {
    cout << "The Graph has " << NumVertices() << " _vertices:" << endl;
    ...
    while (mst.NumVertices() != this->NumVertices()) {
        ...
        for (auto&& j : mst._vertices) {
            for (auto&& i : this->_vertices) {
                ...
                if (ite == this->_edges.end()) continue;
                if (itg != mst._edges.end()) continue;
                bool bflag = false;
                int temp1 = 1;
                int temp2 = 1;
...
void opening() {
    cout << "**             Power grid cost simulation system             **"
         ...
         << "**             5 --- Print Graph                             **"
         << endl
         << "**             6 --- Constructing minimum spanning tree      **"
         << endl
...
void constructMst(Graph<string, int>& g, Graph<string, int>& mst) {
    vertex<string> v;
    string p = "";
